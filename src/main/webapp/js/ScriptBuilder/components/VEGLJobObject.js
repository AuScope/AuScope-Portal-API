...
Ext.define('ScriptBuilder.components.VEGLJobObject', {
    ...
    getScript : function() {
        ...
        var fieldsToInclude = this.jobObject.get('jobParameters');
        ...
        for (var i = 0; i < fieldsToInclude.length; i++) {
            var field = fieldsToInclude[i].get('name');
            var value = fieldsToInclude[i].get('value');
            var type = fieldsToInclude[i].get('type');

            var valueLiteral = value;
            if (type === vegl.models.Parameter.TYPE_STRING) {
                valueLiteral = '\'' + valueLiteral + '\'';
            }

            classText += field + '=' + valueLiteral;
