...
JobList.ControllerURL = "data_service_tool.html";
...
DataService.onLoadException = function(proxy, options, response, e) {
	DataService.hideProgressDlg();
    if (response.status != 0) {
    	DataService.showError("Could not interpret server response "+
            "(most likely your session has expired). "+
            "Please try reloading the page.");
    } else {
    	DataService.showError("Could not retrieve data from the server ("+
            response.statusText+").");
    }
}
...
DataService.onRequestFailure = function(response, request) {
	DataService.showError('Could not execute last request. Status: '+
        response.status+' ('+response.statusText+')');
}


// callback for retrieveFiles action
DataService.onRetrieveFilesResponse = function(response, request) {
    var resp = Ext.decode(response.responseText);
    if (resp.error != null) {
    	DataService.showError(resp.error);
    } else {
...
DataService.showError = function(message) {
	DataService.errorDlg = Ext.Msg.show({
        title: 'Error',
        msg: message,
        buttons: Ext.Msg.OK,
        icon: Ext.Msg.ERROR,
...
DataService.hideProgressDlg = function() {
    if (DataService.progressDlg) {
    	DataService.progressDlg.hide();
    	DataService.progressDlg = undefined;
    }
}

// notifies the user through a progress dialog that data is being retrieved
DataService.showProgressDlg = function() {
    if (!DataService.progressDlg && !JobList.errorDlg) {
    	DataService.progressDlg = Ext.Msg.wait('Retrieving data, please wait...');
    }
}

// retrieves filelist of selected job and updates the Details panel
DataService.updateJobDetails = function() {
