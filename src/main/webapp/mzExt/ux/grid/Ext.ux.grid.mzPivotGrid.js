...
Ext.define('Ext.ux.grid.column.mzPivotColumn', {
    ...
    renderer: function(value, metaData, record, rowIdx, colIdx, store) {
        ...
        if(record.get('rowGroup') > 1 && this.dataIndex == record.get('rowGroupField')){
...
#Example usage:#
    ...
    */
    afterRender: function(comp, options){
        ...
        me.callParent(arguments);
        me.getStore().on('load', me.initPivotGrid, me);
    },
    ...
    * @private 
    */
    initPivotGrid: function(store, records, successful, options){
        var me = this,
            cells = [],
            countCols = 0,
            storeData,
    ...
    */
    getTuples: function(dimensions){
        ...
        tuples.sortByKey('ASC', me.sortAlphanum);
        return tuples;
    },

    /**
    * @private Javascript natural sorting algorithm taken from http://my.opera.com/GreyWyvern/blog/show.dml/1671288
    */
            ...
            if(record.get('rowGroup') > 1){
                ...
                for(j = 0; j < cols; j++){
                    ...
                    if(header){
                        ...
                        o.rows[i][header.id] = summary(data, 'c' + j);
