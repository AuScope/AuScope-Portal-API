...
class VEGLParameters:
    ...
    _paddingMinEasting = None
    _paddingMaxEasting = None
    _paddingMinNorthing = None
    _paddingMaxNorthing = None
    ...
    def __init__(self, paddingMinEasting, paddingMaxEasting, paddingMinNorthing, paddingMaxNorthing, selectionMinEasting, selectionMaxEasting, selectionMinNorthing, selectionMaxNorthing, mgaZone, cellX, cellY, cellZ, inversionDepth):
        self._paddingMinEasting = paddingMinEasting
        self._paddingMaxEasting = paddingMaxEasting
        self._paddingMinNorthing = paddingMinNorthing
        self._paddingMaxNorthing = paddingMaxNorthing
    ...
    def getPaddingMinEasting(self):
        return self._paddingMinEasting

    def getPaddingMaxEasting(self):
        return self._paddingMaxEasting

    def getPaddingMinNorthing(self):
        return self._paddingMinNorthing

    def getPaddingMaxNorthing(self):
        return self._paddingMaxNorthing
    ...
    def getPaddedBounds(self):
        return VEGLBBox(srs=self._mgaZone, maxNorthing=self._paddingMaxNorthing, maxEasting=self._paddingMaxEasting, minNorthing=self._paddingMinNorthing, minEasting=self._paddingMinEasting)

    # Gets an instance of VEGLBBox representing the padded bounds
    def getSelectedBounds(self):
        return VEGLBBox(srs="EPSG:4326", maxNorthing=self._selectionMaxNorthing, maxEasting=self._selectionMaxEasting, minNorthing=self._selectionMinNorthing, minEasting=self._selectionMinEasting)

# Global parameter instance for reference
VEGLParams = VEGLParameters(paddingMinEasting=${job-padded-mineast}, paddingMaxEasting=${job-padded-maxeast}, paddingMinNorthing=${job-padded-minnorth}, paddingMaxNorthing=${job-padded-maxnorth}, selectionMinEasting=${job-selection-mineast}, selectionMaxEasting=${job-selection-maxeast}, selectionMinNorthing=${job-selection-minnorth}, selectionMaxNorthing=${job-selection-maxnorth}, mgaZone='${job-mgazone}', cellX=${job-cellx}, cellY=${job-celly}, cellZ=${job-cellz}, inversionDepth=${job-inversiondepth})

# ----- Autogenerated AWS Utility Functions -----
# Uploads inFilePath to the specified bucket with the specified key
def cloudUpload(inFilePath, cloudBucket, cloudDir,cloudKey):
...
def cloudDownload(cloudBucket, cloudDir,cloudKey, outFilePath):
    queryPath = (cloudBucket + "/" + cloudDir + "/" + cloudKey).replace("//", "/")
...
def main():
    # ------------ VEGL - Step 1 ---------
    f = file(VEGLParams.getVmSubsetFilePath(), "r")
    ...
    VEGLPaddedBox = VEGLParams.getPaddedBounds()
    VEGLSelectedBox = VEGLParams.getSelectedBounds()
    zone = int(VEGLPaddedBox.getSrs())
    temp_data = []
    for x, y, z in data:
    ...
    # Mesh file name: 'mesh'
    VEGLPaddedBox = VEGLParams.getPaddedBounds()
    minEasting = VEGLPaddedBox.getMinEasting()
    maxEasting = VEGLPaddedBox.getMaxEasting()
    minNorthing = VEGLPaddedBox.getMinNorthing()
    maxNorthing = VEGLPaddedBox.getMaxNorthing()
    ...
    cloudUpload(sns_out, VEGLParams.getStorageBucket(), VEGLParams.getStorageBaseKey(), sns_out)
    cloudUpload(inv_out, VEGLParams.getStorageBucket(), VEGLParams.getStorageBaseKey(), inv_out)
    # Upload the mesh file
    cloudUpload(mesh, VEGLParams.getStorageBucket(), VEGLParams.getStorageBaseKey(), mesh)
    ...
    for invFile in invFilesToUpload:
        cloudUpload(invFile, VEGLParams.getStorageBucket(), VEGLParams.getStorageBaseKey(), invFile)
