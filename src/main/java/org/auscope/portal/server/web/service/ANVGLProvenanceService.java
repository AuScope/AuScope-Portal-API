...
public class ANVGLProvenanceService {
    ...
    private static final Log LOGGER = LogFactory.getLog(ANVGLProvenanceService
            .class);
     ...
     * @param newAnvglFileStagingService set the local file store must not
     *                                   be null
     * @param newCloudStorageServices set the cloud file store must not be null
     */
    @Autowired
    public ANVGLProvenanceService(final ANVGLFileStagingService
                                              newAnvglFileStagingService,
                                  final CloudStorageService[]
                                          newCloudStorageServices) {
        this.anvglFileStagingService = newAnvglFileStagingService;
    ...
    protected final void uploadModel(final Model model, final VEGLJob job) {
        if (model != null) {
            try {
                File tmpActivity = anvglFileStagingService.createLocalFile(
