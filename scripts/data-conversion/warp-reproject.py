...
fileset = glob.glob('/projects/r17/GA/*.ers')
...
for currFile in fileset:
    ...
    if not os.path.exists(currFile.replace('r17/GA/','r17/shared/netCDF/').replace('.ers', '.warp.nc4')):
        warp_command = ['gdalwarp',
                   ...
                   '-r', 'cubic',
                   currFile,
                   currFile.replace('r17/GA/','r17/temp/').replace('.ers', '.warp.ers')
                  ]
        translate_command = ['gdal_translate',
                   ...
                   '-co', 'ZLEVEL=9',
                   currFile.replace('r17/GA/','r17/temp/').replace('.ers', '.warp.ers'),
                   currFile.replace('r17/GA/','r17/shared/netCDF/').replace('.ers', '.warp.nc4')
                   ]
        ...
        print currFile.replace('r17/GA/','r17/shared/netCDF/').replace('.ers', '.warp.nc4'), 'already exists'