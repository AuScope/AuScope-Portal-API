...
puppi::netinstall { 'hdf5':
    ...
    extracted_dir => 'hdf5-1.8.9-build',
    destination_dir => '/usr/local',
    postextract_command => '/usr/local/hdf5-1.8.9-build/configure --prefix=/usr/local/hdf5-1.8.9 --enable-cxx --enable-fortran && make all && make install',
}
...
exec {'uw-obtainrepos':
    cwd => "$uwBuildDir",
    command => "obtainRepositories.py --with-gocadtoolbox=1",
...
export HDF5_DIR=/usr/local/hdf5-1.8.9\\
